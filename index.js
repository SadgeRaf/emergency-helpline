    ...
    elements[i].addEventListener('click', function () {
        ...
        if (new_coins >= 20) {
            ...
            for (data of callHistory) {
                ...
                div.className = "w-auto max-w-full bg-gray-100 flex items-center justify-between px-4 py-3 rounded-xl";

                div.innerHTML = `<div>
                                   <h1 class="text-lg font-bold">${historyData.name}</h1>
                                   <p class="text-sm text-gray-600">${historyData.number}</p>
                                 </div>
                                 <p class="text-xs text-gray-500">${historyData.date}</p>`;
